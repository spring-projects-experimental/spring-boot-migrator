...
class ListCommandRendererTest {

    @Mock
    private RecipeRenderer recipeRenderer;

    @InjectMocks
    private ListCommandRenderer sut;

    @Test
    void testRendererWithRecipesFound() {
        AttributedString renderedRecipesList = new AttributedString("");
        List<Recipe> recipeList = Collections.emptyList();
        when(recipeRenderer.renderRecipesList("No recipes found.", "Found these recipes:", recipeList)).thenReturn(renderedRecipesList);
