...
public class GenerateWebServices extends AbstractAction {
    ...
    public static Xml.Document parseWsdl(Path p, ExecutionContext executionContext) {
        Parser.Input o = new Parser.Input(p, () -> {
            try {
        ...
        List<SourceFile> docs = XmlParser.builder().build().parseInputs(Collections.singleton(o), (Path)null, executionContext).toList();
        if (docs.isEmpty()) {
            throw new RuntimeException("Failed to parse XML file '" + p + "'");
        } else {
            return (Xml.Document) docs.get(0);
        }
    }
