...
public class OpenRewriteSourceFilesFinder implements ProjectResourceFinder<List<SourceFile>> {

    public List<SourceFile> apply(ProjectResourceSet projectResourceSet) {
        return projectResourceSet.stream()
                .filter(RewriteSourceFileHolder.class::isInstance)
                .map(RewriteSourceFileHolder.class::cast)
                .filter(pr -> SourceFile.class.isAssignableFrom(pr.getType()))
