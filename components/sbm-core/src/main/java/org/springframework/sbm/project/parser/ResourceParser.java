...
public class ResourceParser {
    ...
    private List<SourceFile> parseSingleResource(Path baseDir, ExecutionContext ctx, Map.Entry<Parser, List<Parser.Input>> e, Parser.Input resource) {
        try {
            return e.getKey().parseInputs(List.of(resource), baseDir, ctx).toList();
        } catch(Exception ex) {
