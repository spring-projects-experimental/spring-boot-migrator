...
public class GitSupportTest {
    @Test
    void findRepository_withoutGit_shouldLogErrorAndReturnEmpty(@TempDir Path tmpDir, CapturedOutput output) {
        ...
        assertThat(output.getAll()).matches("[\\d\\: \\. ]*\\[main\\] DEBUG o\\.s\\.sbm\\.engine\\.git\\.GitSupport - Could not find \\.git in the given directory '%s' or any of it's parents(?s).*".formatted(tmpDir.toString().replace("\\", "\\\\")));
    }

    @Test
    void findRepository_witGitInDir_shouldReturnRepository(@TempDir Path tmpDir) {
        try (Git git = GitSupport.initGit(tmpDir.toFile())) {
            Optional<Repository> repository = GitSupport.findRepository(tmpDir.toFile());
    ...
    void addFiles_withGitInParentDirAndFilenameAsPattern_shouldAddTheFileToIndex(@TempDir Path tmpDir) {
        ...
        try (Git git = GitSupport.initGit(tmpDir.toFile())) {
            ...
            GitStatus status = gitSupport.getStatus(tmpDir.toFile());
            String resolvedFilename = unifyPath(tmpDir.relativize(projectDir.resolve(filename)));
            assertThat(status.getUncommittedChanges()).isNotEmpty().contains(resolvedFilename);
        }
    }
    ...
    void addFiles_withGitInGivenDirAndDotAsPattern_shouldAddTheFileToIndex(@TempDir Path tmpDir) {
        ...
        try (Git git = GitSupport.initGit(tmpDir.toFile())) {
            ...
            GitStatus status = gitSupport.getStatus(tmpDir.toFile());
            assertThat(status.getUncommittedChanges()).contains(unifyPath(tmpDir.relativize(projectDir.resolve(filename))));
        }
    }
    ...
    private static String unifyPath(Path tmpDir) {
        return LinuxWindowsPathUnifier.unifyPath(tmpDir);
    }
}
