...
/**
 * Parse {@code .mvn/maven.config/} and provide access to relevant settings. Code
 * thankfully taken from <a href=
 * "https://github.com/apache/maven/blob/857a5e129ad8dad05495ba631818b55f1925d210/maven-embedder/src/main/java/org/apache/maven/cli/CLIManager.java">org.apache.maven.cli.CLIManager</a>.
 *
...
class MavenConfigFileParser {
	...
	public List<Option> parse(String[] args) {
		...
		options.addOption(Option.builder(PROJECT_LIST)
			.longOpt("projects")
			.desc("Comma-delimited list of specified reactor projects to build instead of all projects. A project can be specified by [groupId]:artifactId or by its relative path")
			.hasArg()
			.build());
