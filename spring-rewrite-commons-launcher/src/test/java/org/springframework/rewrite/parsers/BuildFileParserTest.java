...
class BuildFileParserTest {
	...
	public class GivenSimpleMavenMultiModuleProject {
		...
		void filterAndSortBuildFiles_shouldReturnSortedListOfFilteredBuildFiles() {
			...
			List<Resource> resources = List.of(new DummyResource("src/test/resources/dummy/pom.xml", ""), // filtered
			...
			Assertions.assertThat(ResourceUtil.getPath(resourceList.get(0))).isEqualTo(resolve);
		...
		void parseBuildFiles_shouldReturnSortedListOfParsedBuildFiles() {
			...
			List<Resource> resources = List.of(new DummyResource(module1SubmoduleSourcePath, POM_3),
					new DummyResource(parentSourcePath, POM_1), new DummyResource(module1SourcePath, POM_2));
			...
			List<Xml.Document> parsedBuildFiles = sut.parseBuildFiles(baseDir, resources, List.of("default"),
					executionContext, skipMavenParsing, provenanceMarkers);

			assertThat(parsedBuildFiles).hasSize(3);
			assertThat(parsedBuildFiles.get(0).getMarkers().findFirst(JavaProject.class).get().getProjectName())
				.isEqualTo("module1/submodule");
			assertThat(parsedBuildFiles.get(1).getMarkers().findFirst(JavaProject.class).get().getProjectName())
				.isEqualTo("parent");
			assertThat(parsedBuildFiles.get(2).getMarkers().findFirst(JavaProject.class).get().getProjectName())
				.isEqualTo("module1");
		}
		...
		void parseWithoutBaseDirShouldThrowException() {
			String message = assertThrows(IllegalArgumentException.class, () -> sut.parseBuildFiles(null, List.of(),
					List.of("default"), new InMemoryExecutionContext(), false, Map.of()))
				.getMessage();
		...
		void parseWithEmptyResourcesShouldThrowException() {
			String message = assertThrows(IllegalArgumentException.class, () -> sut.parseBuildFiles(Path.of("."),
					List.of(), List.of(), new InMemoryExecutionContext(), false, Map.of()))
				.getMessage();
